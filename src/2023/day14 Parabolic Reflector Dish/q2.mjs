...
const part2 = () => {
  ...
  const tilt = (dir) => {
    ...
    rocks.forEach((o, i) => {
      ...
      switch (dir) {
        case 'n':
          o.y = i > 0 && rocks[i - 1].x === o.x ? rocks[i - 1].y + 1 : 0
          break
        case 's':
          o.y = i > 0 && rocks[i - 1].x === o.x ? rocks[i - 1].y - 1 : height - 1
          break
        case 'w':
          o.x = i > 0 && rocks[i - 1].y === o.y ? rocks[i - 1].x + 1 : 0
          break
        case 'e':
          o.x = i > 0 && rocks[i - 1].y === o.y ? rocks[i - 1].x - 1 : width - 1
          break
      }
    })
  }
  ...
  while (true) {
    ...
    if (loads.filter((n) => n === l).length > 1) {
      ...
      if (id > 0 && loads[loads.length - 2] === loads[id - 1]) {
        ...
        return loads[id + ((1_000_000_000 - id - 1) % step)]
      }
    }
  }
}
