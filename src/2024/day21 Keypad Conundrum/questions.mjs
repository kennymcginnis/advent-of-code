...
const answer = (codes, robots) => {
  return codes.reduce((res, code) => res + parseInt(code) * execute(numpad, code, robots), 0)
}

const run = () => {
  let data = parseInput(input)
  timer = Date.now()
  console.log({ part: 1, answer: answer(data, 2), time: timingMonitor() })

  timer = Date.now()
  console.log({ part: 2, answer: answer(data, 25), time: timingMonitor() })
}
