...
const testBlueprint = (blueprint, time) => {
  ...
  const search = (time, oreRobots, clayRobots, obsidianRobots, ore, clay, obsidian, geodes) => {
    ...
    //Build geode robot
    if (obsidianRobots > 0) {
      let canBuildGeodeNow = blueprint.geodeCost[0] <= ore && blueprint.geodeCost[1] <= obsidian
      let timeSkip =
        1 +
        (canBuildGeodeNow
          ? 0
    ...
    if (clayRobots < maxRobots.clay) {
      ...
      let timeSkip = 1 + (canBuildClayNow ? 0 : Math.ceil((blueprint.clayCost - ore) / oreRobots)) //Todo maybe add one here
    ...
    if (oreRobots < maxRobots.ore) {
      ...
      let timeSkip = 1 + (canBuildOreNow ? 0 : Math.ceil((blueprint.oreCost - ore) / oreRobots)) //Todo maybe add one here
